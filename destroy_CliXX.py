...
# VPC ID and Name
vpc_id = 'vpc-0d3e00f4444e9ca77'  # Update as needed
vpc_name = 'CLIXXSTACKVPC'

# Deletion sequence

# 1. Attempt to delete remaining network interfaces within the VPC
print(f"Checking for remaining network interfaces in VPC {vpc_id}...")
network_interfaces = ec2_client.describe_network_interfaces(Filters=[{'Name': 'vpc-id', 'Values': [vpc_id]}])
for interface in network_interfaces['NetworkInterfaces']:
    eni_id = interface['NetworkInterfaceId']
    print(f"Found Network Interface: {eni_id}")
    ...
    if 'Attachment' in interface:
        ...
        try:
            print(f"Detaching Network Interface: {eni_id} from Attachment: {attachment_id}")
            ec2_client.detach_network_interface(AttachmentId=attachment_id, Force=True)
            time.sleep(5)  # Wait briefly for detachment
        except ec2_client.exceptions.ClientError as e:
    ...
    for attempt in range(5):
        try:
            ...
            if 'DependencyViolation' in str(e):
                ...
                print(f"Failed to delete Network Interface {eni_id}: {str(e)}")
                break

# 2. Delete any remaining VPC peering connections
print(f"Checking for VPC Peering Connections in VPC {vpc_id}...")
vpc_peering_connections = ec2_client.describe_vpc_peering_connections(
...
for pcx in vpc_peering_connections['VpcPeeringConnections']:
    ...
    time.sleep(5)  # Allow time for deletion

# 3. Attempt to delete the VPC after cleaning up dependencies
for attempt in range(5):
    try:
        ec2_client.delete_vpc(VpcId=vpc_id)
        print(f"VPC '{vpc_name}' (ID: {vpc_id}) deleted successfully.")
        break
    except ec2_client.exceptions.ClientError as e:
        if 'DependencyViolation' in str(e):
            print(f"Retrying VPC deletion due to DependencyViolation...")
            time.sleep(15)
        else:
            print(f"Failed to delete VPC {vpc_id}: {str(e)}")
            break
